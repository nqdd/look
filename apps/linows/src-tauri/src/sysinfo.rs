...
pub fn get_system_info() -> Vec<Vec<SysInfoEntry>> {
    ...
    // Section 1: OS
    {
        ...
        if let Ok(h) = std::fs::read_to_string("/etc/hostname") {
            s.push(SysInfoEntry { label: "Host".into(), value: h.trim().to_string() });
        }
        if let Ok(output) = std::process::Command::new("uname").arg("-r").output() {
            let kernel = String::from_utf8_lossy(&output.stdout).trim().to_string();
            s.push(SysInfoEntry { label: "Kernel".into(), value: kernel });
        }
        if let Ok(release) = std::fs::read_to_string("/etc/os-release") {
            if let Some(name) = release.lines()
                .find(|l| l.starts_with("PRETTY_NAME="))
                .and_then(|l| l.strip_prefix("PRETTY_NAME="))
                .map(|v| v.trim_matches('"').to_string())
            {
                s.push(SysInfoEntry { label: "OS".into(), value: name });
            }
        }
        ...
        if let Ok(meminfo) = std::fs::read_to_string("/proc/meminfo") {
            ...
            for line in meminfo.lines() {
                if let Some(val) = line.strip_prefix("MemTotal:") {
                    ...
                    if cached == 0 { cached = parse_kb(val); }
                }
            }
            if total > 0 {
                let used = total.saturating_sub(available);
                s.push(SysInfoEntry { label: "Total".into(), value: format!("{:.1} GB", total as f64 / 1048576.0) });
                s.push(SysInfoEntry { label: "Used".into(), value: format!("{} MB", used / 1024) });
                s.push(SysInfoEntry { label: "Cached".into(), value: format!("{} MB", cached / 1024) });
            }
        }
        if !s.is_empty() { sections.push(s); }
    }
    ...
    // Section 3: CPU
    {
        ...
        if let Ok(cpuinfo) = std::fs::read_to_string("/proc/cpuinfo") {
            ...
            let model = cpuinfo.lines()
                ...
                .unwrap_or_default();
            if !model.is_empty() {
                s.push(SysInfoEntry { label: "CPU".into(), value: model });
                s.push(SysInfoEntry { label: "Cores".into(), value: format!("{}", cores) });
            }
                ...
                if let Some(line) = stat.lines().find(|l| l.starts_with("cpu ")) {
                    ...
                    if vals.len() >= 4 {
                        ...
                        if total > 0 {
                            ...
                            s.push(SysInfoEntry { label: "Usage".into(), value: format!("{:.1}%", usage) });
                        }
                    }
                }
            }
    ...
    // Section 4: Battery (if laptop)
    {
        ...
        if bat_path.exists() {
            if let Ok(cap) = std::fs::read_to_string(bat_path.join("capacity")) {
                s.push(SysInfoEntry { label: "Charge".into(), value: format!("{}%", cap.trim()) });
            }
            if let Ok(status) = std::fs::read_to_string(bat_path.join("status")) {
                s.push(SysInfoEntry { label: "Status".into(), value: status.trim().to_string() });
            }
        }
        if !s.is_empty() { sections.push(s); }
    }
        ...
        if let Ok(uptime) = std::fs::read_to_string("/proc/uptime") {
            if let Some(secs_str) = uptime.split_whitespace().next() {
                if let Ok(secs) = secs_str.parse::<f64>() {
                    ...
                    s.push(SysInfoEntry { label: "Time".into(), value: val });
                }
            }
        }
        if !s.is_empty() { sections.push(s); }

...
pub(crate) fn list() -> Vec<RunningApp> {
    ...
    for (pid, basename) in enumerate_processes() {
        ...
        let display = resolve_display_name(&basename, &title_str);
        let app = RunningApp {
            name: display,
            pid,
...
pub(crate) fn list_on_port(port: u16) -> Vec<RunningApp> {
    ...
    for pid in pids {
        ...
        let display = resolve_display_name(&basename, &title);
        out.push(RunningApp {
            name: display,
            pid,
            desktop_id: (!exe_path.is_empty()).then(|| format!("app:{exe_path}")),
...
fn resolve_display_name(process_name: &str, window_title: &str) -> String {
